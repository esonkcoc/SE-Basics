...
A file's current state, saved in a project's history, is deemed **unmodified** and requires no further action. At this point, the file can be removed or edited. If removed from the repo, it returns to being **untracked** and will require **staging** and **committing** again. If edited, it is now deemed **modified**, meaning its current state differs from that of the version **committed** to the projects's history. It too can then be **staged** and **committed**.
...
## Git Setup
By default, the .git installation is created hidden within the ***root*** **directory** (/Users/abhi) you chose to create it in. To see hidden files in the ***working*** **directory** execute the command `ls -a /Users/abhi`, where `ls` represents 'lists' and `-a` 'all'. To confirm the path of the ***working*** **directory**  execute the command `pwd`, which represents 'print working directory'. To change the ***working*** **directory** execute the command `cd`, which represents 'change directory', followed by the path to your new ***working*** **directory**, e.g., `cd /Users/abhi/Desktop/programming/notes` (whatever the new path might actually be).

To find the path name of the directory you want to change to:
1. navigate to the folder whose directory path you want to find
2. right-click on the folder
3. hold down the "Option" key to reveal an option in the context menu called "Copy [Folder Name] as Pathname". 

Alternatively, `~/Desktop/programming/notes`, where `~` represents your ***root*** **directory**, relative to your new working directory, can be used. When inputing a path name with potentially awkward charaters, such as '&' or ' ', make sure to enclose the entire path within quotation marks, e.g., `cd ~"/hello/SE & DE Notes"`.  
...
`git add` *alone* **stages** *all* changes in the ***working*** **directory**.
...
To see a log of all the changes made to a repo execute the command `git log`. To see if your local repo is connected to a remote repo the command `git remote -v` will show you the URLs associated with it.
