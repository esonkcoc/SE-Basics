...
If you want to search for a string within an array, your code would require modification as such:
...
One way we judge an algorithm's efficiency is with Big *O* notation, that is the broad sense of time it takes to complete a problem at scale; its running time. Minor details, like the ..., are negligable when considerably scaled up, so much so that an algorithm at the upper bound of runtime may as well be another algorithm also near the upper bound when judging by the shape of line it makes on a graph.
...
<g>w</g>(*n*)       |
<g>w</g>(log *n*)   |
<g>w</g>(1)       V Fastest      Both Linear and Binary search algorithms potentially have a lower running time bound of <g>w</g>(1) if by chance the value they were searching for were first in its array.

Finally, should an algorithm's upper and lower running time bound be identical for *O* and <g>w</g>, it can be said to have an efficiency of Theta(*order*). i.e., No matter what, the algorithm will always take the same number of steps to complete.
...
or, more simply, `*n*2/2 - *n*/2`. Considering that mathematical analysis, *n*2 is really the most influential factor in determining the efficiency of this algorithm. Therefore, selection sort is considered to be of the order of *O*(*n*2) in the worst case where all values are unsorted. Even when all values are sorted, it will take the same number of steps. Therefore, the best case can be noted as <g>w</g>(*n*2). Since both the upper bound and lower bound cases are the same, the efficiency of this algorithm as a whole can be regarded as <g>q</g>(*n*2). 
...
Analysing bubble sort, the worst case is *O*(*n*2), while the best case is <g>w</g>(*n*). 
...
Merge sort is a very efficient sort algorithm with a worst case of *O*(*n* log *n*). The best case is still <g>w</g>(*n* log *n*) because the algorithm still must visit each place in the list. Therefore, merge sort is also <g>q</g>(*n* log *n*) since the best case and worst case are the same.
...
In the example below, the `draw` function calls itself. Also, the programme may get caught in an infinite loop because there is nothing telling it to end, i.e., no case where the programme is done. To break from this loop return `ctrl-c`.
...
In the example below, the base case will ensure the code does not run forever. The line `if (n <= 0)` terminates the recursion because the problem has been solved. Every time `draw` calls itself, it calls itself by `n-1`. At some point, `n-1` will equal 0, resulting in the `draw` function returning and the programme will end.
